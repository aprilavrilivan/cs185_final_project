...
def compute_offline_preference_loss(
    *,
    ...
    if algo == "dpo":
        ...
        chosen_rewards = (policy_scores.chosen_logp_sum - reference_scores.chosen_logp_sum).sort().values
        rejected_rewards = (policy_scores.rejected_logp_sum - reference_scores.rejected_logp_sum).sort().values
        quantile_gap = chosen_rewards - rejected_rewards
        losses = F.logsigmoid(quantile_gap * beta).mul(-1.0)
        metrics.update(
            {

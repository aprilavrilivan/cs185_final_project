    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                seq_log_ratio = masked_mean_per_row(new_logp - mb.old_logprobs, mask)
                seq_log_ratio = torch.clamp(seq_log_ratio, -20.0, 20.0)
                sequence_ratio = torch.exp(seq_log_ratio)
                unclipped_t = sequence_ratio * adv
                clipped_t = torch.clamp(sequence_ratio, 1 - cfg.clip_eps, 1 + cfg.clip_eps) * adv
                seq_obj_i = torch.min(unclipped_t, clipped_t)
                pg_loss = -seq_obj_i.mean()
                kl = approx_kl_from_logprobs(new_logp, mb.ref_logprobs, mask)
                entropy = -masked_mean(new_logp, mask)
                clipfrac = ((sequence_ratio > (1 + cfg.clip_eps)) |
                            (sequence_ratio < (1 - cfg.clip_eps))).float().mean()
